...
class OptimalAlignment:
    ...
    def get_aligned_sequences(self, sequence_format="list"):
        ...
        """
        if sequence_format == "list_of_int":
            seq_a = [int(str(x)) for x in self._change_gap_char(self._alseq1)]
            seq_b = [int(str(x)) for x in self._change_gap_char(self._alseq2)]
        elif sequence_format == "list":
            ...
            raise ValueError("Sequence_format has to be either 'list_of_int' 'list' or 'str'!")
        return seq_a, seq_b
